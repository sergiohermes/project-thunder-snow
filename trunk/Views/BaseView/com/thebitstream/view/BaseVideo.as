package com.thebitstream.view {
	...
	public class BaseVideo extends MovieClip implements IProject {
		...
		public function BaseVideo()
		{
			...
			txtHtml.htmlText="<a href='http://thundersnow.thebitstream.com'>Thunder Snow</a>"
			txtDesc.text="";
			txtName.text="";
		...
		{
			presentation=prov;
			presentation.x=(360/2 - pWidth/2) + 6
			presentation.y=12;
			//add to the screen layer
		...
		public function set providerHeight(val:Number):void
		{
			...
			if(val){
				if(presentation)
				presentation.y=240/2 - pHeight/2 +12
			}
		}
		/**
		...
		public function onScriptData(val:Object):void
		{
			switch(val.name)
			{
				case "onSubt":
					trace("Subtitle\n",val.value.subtitle);
					break;
				case "onAsyn"://stream segment end.	
